...
<header class="header">
      ...
      <div class="bars"></div>
			...
			<nav class="menu">
                <div class="btn-menu">
				<b><a href="tenis.php">Nuevos lanzamientos</a></b>
        ...
        <a href="mis_compras.php"><img src="img/mis_compras.png" width="50"><span id="num_cart" class="badge bg-secondary"></a>
   
        <a href="cerrar_sesion.php">Cerrar sesión</a>
        <a href=""><?php echo $rol; ?></a>    
        <?php
        $es_admin = ($rol === 'administrador'); // Verificar si el rol del usuario es 'administrador'
          if ($es_admin) {
          echo '<select id="admin" style="width: 25px;" onchange="pagAdm()">
          <option value="" disabled selected hidden>Opciones</option>
          <option value="cuentas.php">Cuentas</option>
          <option value="mis_productos.php">Mis Productos</option>
          <option value="compras.php">Compras</option>
          <option value="detalle_compra.php">Detalle Compra</option>
        </select>';
        }
        ?>
  ...
  <center>
    <h1 style="margin: 10px; color:white;">Bienvenido <?php echo $nombre; ?>!</h1> <!-- Mostrar el nombre -->
    <h5 style="color:white">Te mostramos los nuevos lanzamientos</h5>
    </center>
<main>
...
<script>
  function pagAdm() {
    ...
    if (selectedValue !== "") {
      // Agregar una condición para permitir al usuario administrador seleccionar opciones
      if ('<?php echo $rol; ?>' === 'administrador') {
        window.location.href = selectedValue;
      } else {
        alert("No tienes privilegios de administrador para acceder a esta opción.");
        selectElement.selectedIndex = 0; // Restablecer el valor seleccionado a la opción predeterminada
      }
    }
  }
</script>
